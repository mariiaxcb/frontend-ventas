...
const TEMP_TOKEN =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwidXNlcm5hbWUiOiJBZG1pbiIsImlhdCI6MTc4OTUxODQ4NSwiZXhwIjoxNzg5NjA0ODg1fQ.apKUUim4fyMEpJ9ApV5P_RUmtylSvKa7BUXawLZN3ak'
...
export default function TransitionLivePage() {
  ...
  const [processingProductCode, setProcessingProductCode] = useState<
    string | null
  >(null)
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (jsonResponse.success && jsonResponse.data) {
            ...
            setIsConnected(true)
            if (
              jsonResponse.data._count &&
              typeof jsonResponse.data._count.orders === 'number'
            ) {
              setDbSalesCount(jsonResponse.data._count.orders)
            }
            if (
              jsonResponse.data.products &&
              Array.isArray(jsonResponse.data.products)
            ) {
  ...
  useEffect(() => {
    ...
    socketInstance.on(
      'nueva_intencion_compra',
      (newComment: FilteredComment) => {
        setComments((prev) => {
  ...
  const toggleProductInStream = async (product: Product) => {
    ...
    try {
      if (isAlreadyAdded) {
        ...
        if (res.ok) {
          setStreamProducts((prev) =>
            prev.filter((p) => p.code !== product.code),
          )
        }
      } else {
        const res = await fetch(
          `${BACKEND_URL}/api/streams/${activeStream.id}/products`,
          {
            method: 'POST',
            headers: {
  ...
  const handleCreateStream = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`${BACKEND_URL}/api/tiktok/streams`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${TEMP_TOKEN}`,
      ...
      if (response.ok) {
        ...
        alert(`Error: ${data.error || 'No se pudo crear la transmisión'}`)
      }
    } catch (error) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            {!activeStream
              ? 'Inicia una nueva transmisión configurando los datos principales.'
        ...
        {activeStream && (
          <div className="flex items-center gap-3">
            ...
            <div className="bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-lg text-xs text-slate-300">
              Ventas (BD):{' '}
              <span className="font-bold text-emerald-400">{dbSalesCount}</span>
            </div>
          </div>
        )}
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(80vh-10rem)] min-h-[600px]">
            ...
            <div className="h-full">
              <ListaPostulantes
                onVentaConfirmada={() => setDbSalesCount((prev) => prev + 1)}
              />
            </div>
          </div>
        </>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {isFetchingProducts ? (
                ...
                filteredProducts.map((product) => {
                  const isAdded = streamProducts.some(
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <img
                          ...
                          alt={product.name}
                          className="w-12 h-12 rounded-md object-cover bg-slate-900"
                        />
                        <div>
